...
module.exports.getGroupMessages = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id))
      return res.status(400).json({ message: "ID Nhóm không tồn tại" });
    ...
    const messages = await Message.find({
      ...
      .sort({ createdAt: 1 })
      // dùng populate để lấy thông tin của senderId và seenBy
      .populate([
        { path: "senderId", select: "username avatar fullName" },
        { path: "seenBy", select: "username avatar fullName" },
      ]);

    return res
      .status(200)
      .json({ message: "Lấy tin nhắn thành công", messages });
  } catch (error) {
